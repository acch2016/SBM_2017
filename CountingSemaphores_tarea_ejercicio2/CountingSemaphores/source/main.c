...
//sera binary
SemaphoreHandle_t xBinarySemaphore_blue;
SemaphoreHandle_t xBinarySemaphore_green;
SemaphoreHandle_t xBinarySemaphore_red;
...
void PORTA_IRQHandler(void)
{
	...
	if (1 == counter) {
		...
		xSemaphoreGiveFromISR(xCountingSemaphore_blue,
				&xHigherPriorityTaskWoken);
	} else if (2 == counter) {
		xSemaphoreGiveFromISR(xCountingSemaphore_green,
				&xHigherPriorityTaskWoken);
	} else if (3 == counter) {
		xSemaphoreGiveFromISR(xCountingSemaphore_red,
				&xHigherPriorityTaskWoken);
...
int main(void) {
 ...
 //se crea
  vSemaphoreCreateBinary( xBinarySemaphore_blue );
  vSemaphoreCreateBinary( xBinarySemaphore_green );
  vSemaphoreCreateBinary( xBinarySemaphore_red);
